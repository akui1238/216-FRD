...
class DriveDataset(Dataset):
    def __init__(self, root: str, train: bool, transforms=None):
        ...
        # 根据训练或测试情况生成不同的标注文件路径
        if train:
            self.manual = [os.path.join(data_root, "1st_manual", i.split("_")[0] + "_manual1.gif")
                           for i in img_names]
        else:
            self.manual = [os.path.join(data_root, "1st_manual", i.split("_")[0] + "_test_mask.gif")
                           for i in img_names]
...
class STAREDataset(Dataset):
    ...
    def __len__(self):
        if self.train :return len(self.img_list)*8
        else :return len(self.img_list)
...
class HRFDataset(Dataset):
    def __init__(self, root: str, train: bool, transforms=None):
        ...
        assert os.path.exists(data_root), f"path '{data_root}' does not exists."
        self.transforms = transforms
        img_names = [i for i in os.listdir(os.path.join(data_root, "images")) if i.endswith((".jpg", ".JPG"))]
        self.img_list = [os.path.join(data_root, "images", i) for i in img_names]
