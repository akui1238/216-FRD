...
from models.DCNv4_op.DCNv4 import DCNv4
...
class DoubleConv(nn.Module):  # 576 288
    def __init__(self, in_channels, out_channels, num=None, dilation=None):
        ...
        # 替换普通卷积为DCNv4
        self.dcn1 = DCNv4(
            channels=out_channels, kernel_size=3, stride=1, pad=1,
            group=4, center_feature_scale=True
        )
...
class FRD_Net(nn.Module):
    def __init__(self,
        ...
        self.conv5_1 = DoubleConv(base_c, base_c, num=num, dilation=1)
        self.conv5_2 = DoubleConv(base_c, base_c, num=num, dilation=1)

        self.decoder4 = DoubleConv(base_c, base_c, num=num, dilation=1)
    ...
    def forward(self, x):
        ...
        # 第五层
        out = self.conv5_1(out)  # b, c5, H/4, W4
        out = self.conv5_2(out)  # b, c4, H/4, W/4
        out = F.interpolate(out, size=t4.shape[2:], mode='bilinear', align_corners=False)

        # 第四层
        out=self.GRU3(out,t4)
        out = self.decoder4(out)
        out = F.interpolate(out, size=t3.shape[2:], mode='bilinear', align_corners=False)

        # 第三层
        out=self.GRU2(out,t3)
        out = self.decoder3(out)
        out = F.interpolate(out, size=t2.shape[2:], mode='bilinear', align_corners=False)

        # 第二层
        out=self.GRU1(out,t2)
